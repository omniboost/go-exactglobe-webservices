...
func TestDocumentTypeRead(t *testing.T) {
	req := client.NewDocumentTypeReadRequest()
	req.QueryParams().Top.Set(100)
	...
	for _, t := range resp.D.Results {
		fmt.Printf("%d: %s\r\n", t.ID, t.TypeDescription)
	}
}
